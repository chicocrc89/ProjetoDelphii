...
begin
  ...
  self.umgrupo.setCadastro(Date);
  self.umgrupo.setUlt_Alt(Date);
  self.umFrmCadastroGrupo.limpaedt;
  self.umFrmCadastroGrupo.conhecaObj(umgrupo, umactrlgrupo);
  self.umFrmCadastroGrupo.ShowModal;
  self.Pesquisar;
...
begin
  inherited;
  mMSG := umactrlgrupo.Pesquisar(self.edtPesquisar.Text);
  if mMSG <> '' then
    ShowMessage('GRUPO' + self.edtPesquisar.Text + 'NÃO ENCONTRADO');
end;
