...
begin
  ...
  umfrmconsultafuncionario.ConhecaObj(umusuario.getumfuncionario, umactrlusuario.getctrlfuncionario);
...
begin
  ...
  self.edtcodfuncionario.Text := inttostr(umusuario.getumfuncionario.getCodigo);
  self.edtfuncionario.Text := umusuario.getumfuncionario.getnome;
  self.edtCadastro.Text := DateToStr(umusuario.getCadastro);
  self.edtUlt_alt.Text := DateToStr(umusuario.getUlt_Alt);
...
begin
  ...
  self.edtcodusuario.Text := inttostr(self.umusuario.getCodigo_Usuario);
  self.edtCadastro.Text := DateToStr(self.umusuario.getCadastro);
  self.edtUlt_alt.Text := DateToStr(self.umusuario.getUlt_Alt);
end;
...
procedure TFrmCadastroUsuario.edtcodfuncionarioExit(Sender: TObject);
var
  mMSG: string;
begin
  inherited;
  mMSG := umactrlusuario.getctrlfuncionario.Pesquisar(self.edtcodfuncionario.Text);
  umactrlusuario.getctrlfuncionario.Carregar(umusuario.getumfuncionario);
  self.edtcodfuncionario.Text := inttostr(umusuario.getumfuncionario.getcodigo);
  self.edtfuncionario.Text := umusuario.getumfuncionario.getnome;
  if self.edtcodfuncionario.Text = inttostr(0) then
  begin
    ShowMessage('Funcionario não encontrado');
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('Campo Confirmaçao de Senha precisa ser preenchido!!!');
    ...
    begin
      ShowMessage('Campo Permissão precisa ser selecionado!!!');
      self.permissao.SetFocus;
    end
    else if not(self.edtsenha1.Text = self.edtsenha2.Text) then
    begin
    ...
    begin
      ...
      umusuario.setCadastro(StrToDate(self.edtCadastro.Text));
      umusuario.setUlt_Alt(StrToDate(self.edtUlt_alt.Text));
      umusuario.setCodigo_Usuario(strtoint(self.edtcodusuario.Text));
